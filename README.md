...
To build the final jar and optimize the playgroundService application for production, run:
...
` ./mvnw verify -DskipTests=true -Pdependency -X`
Clean cache:\
`./mvnw dependency-check:purge -DskipTests=true -Pdependency`

The "dependency check report" is generated in the `\target\dependency-check-report.html` file.

# Known Issues

There have been reported issues with Serialization, when using certain JDK 11 distributions when using Microsoft Windows.
In this case please use **Oracle JDK 11.0.2**
This JDK version can be downloaded [here](https://www.oracle.com/java/technologies/javase/jdk11-archive-downloads.html).
