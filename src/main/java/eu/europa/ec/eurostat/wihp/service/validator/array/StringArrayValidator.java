...
@Component
public class StringArrayValidator implements FilterValidator {
    ...
    public Boolean apply(final JsonNode fieldSchema) {
        ...
        return TYPE_ARRAY.equals(type.textValue()) && arrayType != null && TYPE_STRING.equals(arrayType.textValue());
    }

    @Override
    public List<ValidationError> validate(final JsonNode fieldValue, final String fieldName, final JsonNode fieldSchema) {

        List<String> values = getArrayNodeValues(fieldValue);
        ...
        if (validationErrors.isEmpty()) {
            return values.stream()
                ...
                .map(value-> CommonValidator.validateStringValue(value, fieldName, fieldSchema))
    ...
    protected List<String> getArrayNodeValues(final JsonNode jsonArray) {
