...
@Component
public class BooleanValidator implements FilterValidator {

    public Boolean apply(final JsonNode fieldSchema) {
        JsonNode type = Optional.of(fieldSchema)
            .map(f -> f.get(TYPE))
            .orElseThrow();
        return TYPE_BOOLEAN.equals(type.textValue());
    }
    ...
    public List<ValidationError> validate(final JsonNode fieldValue,final String fieldName, final JsonNode fieldSchema) {
        ...
        if (!fieldValue.isNull()) {
            final Boolean value = fieldValue.asBoolean();
            return CommonValidator.validateBooleanValue(value, fieldName, fieldSchema);
        } else {
            return CommonValidator.validateBooleanValue(null, fieldName, fieldSchema);
        }
    }
